...
function extractYouTubeId(url: string): string | null {
  ...
  // If it's already a raw ID (11 chars)
...
function HomeView(props: HomeViewProps) {
  ...
  // Determine hero mode: YouTube video or fallback to image
  const videoId =
    config.heroVideo?.enabled && config.heroVideo?.youtubeUrl
      ? extractYouTubeId(config.heroVideo.youtubeUrl)
      : null

  const showVideoHero = videoId !== null

...
import { MessageCircle, Star, Flame, Store, ArrowLeft, ShoppingCart } from 'lucide-react'
...
export function SingleProductTemplate() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // 1. Config du template mono-produit
        ...
        // 2. Produit + images + variants + reviews + faqs en parallèle
    ...
    <div className="min-h-screen bg-white pb-24 md:pb-0">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              {avgRating > 0 && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-500">{avgRating.toFixed(1)} ({data.reviews.length} avis)</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-baseline gap-3">
              <span className="text-4xl font-black text-green-600 md:text-5xl">
              ...
              {discount > 0 && (
                <span className="rounded-full bg-red-100 px-2.5 py-1 text-sm font-bold text-red-600">
                  -{discount}%
                </span>
              )}
            </div>
            ...
            {/* Urgence stock */}
            {lowStock && (
              <div className="flex items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5 text-sm font-semibold text-red-600">
            ...
            <Button
              size="lg"
              className="hidden w-full gap-3 rounded-2xl bg-green-500 py-6 text-lg font-bold text-white shadow-lg shadow-green-500/20 transition-all hover:bg-green-600 active:scale-[0.98] md:flex"
              onClick={handleOrder}
            >
      ...
      {product.description && (
        <section className="mx-auto max-w-3xl px-4 py-8">
          <h2 className="mb-4 text-xl font-bold text-gray-900 md:text-2xl">Description</h2>
      ...
      {data.faqs.length > 0 && (
        <section className="mx-auto max-w-3xl px-4 py-8">
          <h2 className="mb-4 text-xl font-bold text-gray-900 md:text-2xl">Questions fréquentes</h2>
          <FAQAccordion faqs={data.faqs} />
        </section>
      )}

      {/* ─── SECTION 8: Produits similaires (cross-sell) ─── */}
      {data.crossSellProducts.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 py-8">
          <h2 className="mb-4 text-xl font-bold text-gray-900 md:text-2xl">Les clients ont aussi acheté</h2>
      ...
      {/* ─── SECTION 9: Sticky CTA (mobile uniquement) ─── */}
      <StickyCTA

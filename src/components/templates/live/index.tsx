...
export function LivePulseTemplate() {
  ...
  if (!publicShop) {
    return (
      <div className="min-h-screen live-page-bg">
        ...
        <main className="flex-1 max-w-7xl mx-auto w-full px-4 lg:px-6 py-8 md:py-12">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 rounded-2xl bg-white/70 backdrop-blur-md border border-white/60 text-sm text-[#1A1A2E] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FF6154]/30 focus:border-[#FF6154]/50 focus:bg-white/90 transition-all duration-300 min-h-[44px] shadow-sm"
              />
              <AnimatePresence>
                {searchQuery && (
                  <motion.button
            ...
            {/* Result count — glass effect */}
            <div className="hidden sm:flex items-center gap-2 px-4 py-3 rounded-2xl bg-white/70 backdrop-blur-md border border-white/60 text-sm text-gray-500 shrink-0 min-h-[44px] shadow-sm">
            ...
            >
              <motion.button
                ...
                className={`shrink-0 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 min-h-[40px] ${
                  !activeCategory
                    ? 'bg-gradient-to-r from-[#FF6154] to-[#FF9A44] text-white shadow-md shadow-[#FF6154]/20'
                    : 'bg-white/70 backdrop-blur-sm text-gray-600 border border-white/60 hover:bg-white/90 hover:border-gray-300'
                }`}
              >
                Tout
              </motion.button>
          ...
          <AnimatePresence mode="wait">
            {filteredProducts.length > 0 ? (
              ...
              >
                <div className="w-20 h-20 rounded-2xl bg-white/60 backdrop-blur-sm flex items-center justify-center mb-5 shadow-sm">

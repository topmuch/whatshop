...
export function mapShopToAuthShop(shop: Record<string, unknown>): AuthShop {
  return {
    ...
    facebookConnected: (shop.facebookConnected as boolean) ?? false,
    facebookPageId: (shop.facebookPageId as string) ?? undefined,
    facebookPageName: (shop.facebookPageName as string) ?? undefined,
    catalogEnabled: (shop.catalogEnabled as boolean) ?? false,
    catalogProductCount: (shop.catalogProductCount as number) ?? 0,
    trackPageViews: (shop.trackPageViews as boolean) ?? true,
    trackProductViews: (shop.trackProductViews as boolean) ?? true,
    trackWhatsAppClicks: (shop.trackWhatsAppClicks as boolean) ?? true,
  }
}